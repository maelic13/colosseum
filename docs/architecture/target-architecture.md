...
give every package narrower ownership. ADR-0008 binds the concrete Colosseum
package names through implementation. An optional Phase 9.0 rename cannot
change these roles or dependency edges without a new architecture ADR:
...
The `colosseum-cli` binary's `main` parses CLI/run files, selects or creates the run directory
...
| `branding` | GUI presentation/platform policy; each composition root owns its concrete Colosseum product identity | Remove product/path constants from core |
...
assertions and assigns each one to an implementation step. They use the
concrete package names but enforce architectural roles, not brand spelling.
...
| CLI is headless and separate | `cargo tree -p colosseum-cli` rejects `colosseum-gui`, eframe, egui and GUI configuration features; a source-edge check rejects either root importing the other | 2.2 architecture check |
...
performs the integrated review; ADR-0008 binds coherent Colosseum implementation
names without reopening these responsibility boundaries. Phase 9.0 may review
